...
int64_t time::strptime(const std::string &time_str, const std::string &format) {
    int64_t nano = 0;
    std::string normal_time_str = time_str;
    ...
    if (std::regex_search(format, nano_format_regex)) {
        ...
        auto nano_begin = std::sregex_iterator(normal_time_str.begin(), normal_time_str.end(), nano_regex);
        auto nano_end = std::sregex_iterator();
        for (std::sregex_iterator i = nano_begin; i != nano_end; ++i) {
            std::smatch match = *i;
            nano = stol(match.str());
            normal_time_str = std::regex_replace(time_str, nano_regex, "");
        }
    }
