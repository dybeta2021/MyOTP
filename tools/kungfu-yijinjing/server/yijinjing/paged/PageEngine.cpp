...
#ifdef _WINDOWS
inline void usleep(const int &x) {
...
//
//using spdlog::logger;
//std::shared_ptr<spdlog::logger> get_console_logger(const std::string &path, const std::string &logger_name) {
//
...
void signal_callback(int signum) {
    ...
    if (is_task_running.load(std::memory_order_relaxed)) {
        SPDLOG_INFO("PageEngine Caught signal: {}, stopping...", signum);
...
PageEngine::PageEngine(const string &commFileName, const std::string &temp_page_file) : commBuffer(nullptr),
...
bool PageEngine::reg_client(string &_commFile, int &fileSize, int &hashCode, const string &clientName, int pid,
                            bool isWriter) {
...
void PageEngine::release_page(const PageCommMsg &msg) {
    SPDLOG_INFO("[RmPage] (folder) {} (jname) {} (pNum) {} (lpNum) {}", msg.folder, msg.name, msg.page_num,
                msg.last_page_num);
...
byte PageEngine::initiate_page(const PageCommMsg &msg) {
    SPDLOG_INFO("[InPage] (folder) {} (jname) {} (pNum) {} (lpNum) {}", msg.folder, msg.name, msg.page_num,
                msg.last_page_num);
...
void PageEngine::exit_client(const string &clientName, int hashCode, bool needHashCheck) {
    ...
    if (needHashCheck && hashCode != info.hash_code) {
        SPDLOG_INFO("[RmClient] HASH FAILED.. (name) {} (serverHash) {} (clientHash) {}", clientName, info.hash_code,
                    hashCode);
        return;
    }
