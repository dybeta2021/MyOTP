...
Page::Page(void *buffer) : frame(ADDRESS_ADD(buffer, PAGE_INIT_POSITION)), buffer(buffer), position(PAGE_INIT_POSITION),
                           frameNum(0), pageNum(-1) {}
...
PagePtr Page::load(const string &dir, const string &jname, short pageNum, bool isWriting, bool quickMode) {
    ...
    void *buffer = PageUtil::LoadPageBuffer(path, JOURNAL_PAGE_SIZE, isWriting,
