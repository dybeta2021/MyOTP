...
PageProvider::PageProvider(const string &clientName, bool isWriting) : is_writer(isWriting), client_name(clientName),
                                                                       comm_buffer(nullptr), hash_code(0) {
    register_client();
}
...
void PageProvider::register_client() {
    ...
    if (rsp->type == req.type && rsp->success) {
        comm_buffer = PageUtil::LoadPageBuffer(string(rsp->comm_file), rsp->file_size, true,
                                               false /*server lock this already*/);
    } else {
        throw std::runtime_error("cannot register client: " + client_name);
    }
}

void PageProvider::exit_client() {// send message to say good bye
    PagedSocketRequest req = {};
