...
void TraderClient::connect(char frontAddress[]) {
    ptr = CThostFtdcTraderApi::CreateFtdcTraderApi(".//ctp_spot_td_flow/");//必须提前创建好flow目录
...
py::dict get_static_data(
    ...
    bool init_log = flow::utils::init_logger("info", "clogs/static_data.log", false, false, false);
    SPDLOG_INFO("CTP-Spot获取股票ETF期权静态数据！");

    std::string dir_path = "ctp_spot_td_flow";
...
PYBIND11_MODULE(app_ctp_spot_static_data, m) {
