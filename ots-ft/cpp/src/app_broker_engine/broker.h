...
namespace broker_engine {
    // TODO:修改为信号量
    using ots::utils::timing_ms;

    class BrokerCTP : public ots::broker::Broker {
    private:
        ctp_future::CtpTradeClient *client_ = nullptr;

    public:
        BrokerCTP() = default;
        ~BrokerCTP() = default;
        void SetGateway(ctp_future::CtpTradeClient *client);
...
//        void OnQueryPosition() override;
    ...
    inline void BrokerCTP::SetGateway(ctp_future::CtpTradeClient *client) {
        client_ = client;
    }

    inline int BrokerCTP::Login(const ots::broker::Config &config) {
        config_ = config;
    ...
    inline void BrokerCTP::OnQueryAccount(const ots::broker::Account &account) {
        account_ = account;
    ...
    inline int BrokerCTP::QueryPosition() {
